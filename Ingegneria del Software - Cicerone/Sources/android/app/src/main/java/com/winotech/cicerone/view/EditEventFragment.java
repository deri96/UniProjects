...
public class EditEventFragment extends Fragment implements Serializable {
    ...
    // puntatore all'evento da modificare
    private Event event;
     ...
     */
    public static EditEventFragment newInstance(DBManager db, Event event) {

        EditEventFragment fragment = new EditEventFragment();

        Bundle bundle = new Bundle();
        bundle.putSerializable(Constants.DB_KEY, db);
        bundle.putSerializable(Constants.KEY_EVENT_ID, event);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        event = (Event) getArguments().getSerializable(Constants.KEY_EVENT_ID);
        ...
        String startDateText = event.getStartDate().get(GregorianCalendar.DAY_OF_MONTH) + "/" +
                (event.getStartDate().get(GregorianCalendar.MONTH) + 1) + "/" +
                event.getStartDate().get(GregorianCalendar.YEAR);
        startDateView.setText(startDateText);

        // definizione della data di fine dell'evento
        String endDateText = event.getEndDate().get(GregorianCalendar.DAY_OF_MONTH) + "/" +
                (event.getEndDate().get(GregorianCalendar.MONTH) + 1) + "/" +
                event.getEndDate().get(GregorianCalendar.YEAR);
        endDateView.setText(endDateText);

        // definiizone della descrizione dell'evento
        descriptionEditText.setText(event.getDescription());

        // definizione del numero massimo di iscritti
        maxSubscriptionsEditText.setText(Integer.toString(event.getMaxSubscribeds()));

        // definizione della stringa del range
        String range = getResources().getString(R.string.event_start_and_end_date_description_edit) + " ";
        GregorianCalendar dateInCalendar = new GregorianCalendar();
        dateInCalendar.setTime(event.getTour().getStartDate().getTime());
        ...
        dateInCalendar.setTime(event.getTour().getEndDate().getTime());
        range += " - " + dateInCalendar.get(GregorianCalendar.DAY_OF_MONTH) + "/" +
                (dateInCalendar.get(GregorianCalendar.MONTH) + 1) + "/" +
                dateInCalendar.get(GregorianCalendar.YEAR);
        dateDescriptionTextView.setText(range);

        // acquisizione della lista delle lingue disponibili
        final ArrayList<Language> languageList = db.getLanguageList();

        // acquisizione della lista delle lingue gia impostate
        ArrayList<Language> alreadyChosenLanguageList = event.getEventLanguages();

        // definizione delle lingue gia settate
        for(Language language : languageList){

            // inizializzazione del flag di impostazione
            boolean isSet = false;

            // scandendo ogni linguaggio scelto
            for(Language chosenLanguage : alreadyChosenLanguageList){

                // se il linguaggio scelto analizzato è uguale
                // a quello della lista totale allora imposta il flag a true
                if(chosenLanguage.getName().equals(language.getName())) {

                    isSet = true;
                }
            }

            // impostazione della lingua scelta con il relativo flag
            chosenLanguages.put(language.getName(), isSet);
        }
            ...
            public void onClick(View view) {
                ...
                try {
                    ...
                    if (eventMaxSubs >= 0 && eventStartDate != null && eventEndDate != null && !languageIDs.isEmpty()) {
                        ...
                        boolean success = tourController.editEvent(event.getTour(), event.getId(),
                                eventDescription, eventMaxSubs,
                        ...
                        if(success) {
                            ...
                            tourController.getTourFromID(event.getTour().getId());

                            // acquisizione del puntatore al generalController
                            GeneralController generalController = GeneralController.getInstance();

                            // per ogni iscritto si crea una notifica per avvisarlo della modifica dell'evento
                            for(Subscribed subscribed : event.getEventSubscribeds()) {

                                generalController.saveNotification(
                                        db.getMyAccount().getUsername(),
                                        subscribed.getGlobetrotter().getUsername(),
                                        7,
                                        event.getId(),
                                        "NULL");
                            }
