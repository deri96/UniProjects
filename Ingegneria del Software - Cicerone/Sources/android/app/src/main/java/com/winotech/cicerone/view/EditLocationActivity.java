...
public class EditLocationActivity extends FragmentActivity implements OnMapReadyCallback, Serializable {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final int tourID = (int)getIntent().getExtras().getSerializable("TOUR_ID");
        final String cicerone = (String) getIntent().getExtras().getSerializable("CICERONE");

        // inizializzazione della lista di tappe
        markers = new LinkedList<>();

        // acquisizione del puntatore all'istanza di controller dei tour
        ...
        // settaggio del listener per il bottone si submit
        submitButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                // per ogni marker impostato
                for (Marker m : markers){

                    // definisci una nuova tappa acquisendone
                    // latitudine, longitudine e nome
                    Location location = new Location();
                    String name = null;
                    location.setLatitude((float) m.getPosition().latitude);
                    location.setLongitude((float) m.getPosition().longitude);

                    // definizione di un oggetto geocode
                    Geocoder geo = new Geocoder(getApplicationContext(), Locale.getDefault());

                    try {

                        // acquisizione dell'indirizzo associato alla tappa
                    ...
                    // inserimento della tappa formattata nella lista
                    locations.put(name, location);
                }

                // salvataggio dei dati delle tappe
                boolean success = tourController.editLocation(tourID, locations);

                // se il salvataggio va a buon fine allora torna indietro
                if(success) {
                    ...
                    HashMap<String, Object> tempData = tourController.getTourFromID(tourID);
                    ArrayList<Event> tempEvents = (ArrayList<Event>)tempData.get("events");
                    final HashMap<String, Integer> subscribeds = new HashMap<>();

                    // se c'è almeno un evento
                    if(!tempEvents.isEmpty()) {

                        // per ogni evento acquisito
                        for(Event event : tempEvents) {

                            // se l'evento inizia un giorno futuro successivo ad oggi
                    ...
                    intent.putExtra(Constants.FRAGMENT_TO_LOAD_KEY, Constants.SHOW_TOUR_CICERONE_FRAGMENT_VALUE);
