...
function mission_wilson (page)
{
    ...
    {
        actionLevel ("SCONOSCIUTO              La sua scelta è stata corretta.");
    }
    else if (page === 25 || page === 26)
    {
        actionLevel ("SCONOSCIUTO              E' stato l'androide, il suo sospettato, ad aver ucciso Caine. L'ho visto io stesso dai suoi occhi.");
    }
    else if (page === 27 || page === 28)
    {
        actionLevel ("SCONOSCIUTO              Non so se i miei aiuti le sono stati utili ma sono felice comunque del risultato.");
    }
    else if (page === 29 || page === 30)
    {
        actionLevel ("TU                       Quali aiuti? Ma lei chi è?");
    }
    else if (page === 31 || page === 32)
    {
        actionLevel ("SCONOSCIUTO              Diciamo che sono un suo grande fan. Si ricordi del Grande Fratello, un giorno. Ci rivedremo presto.");
    }
    ...
    {
        actionLevel ("TU                       Si si, certo. Qualcuno aveva voglia di scherzare... Bene, è ora di festeggiare, Watson!");
    }
    else if (page === 39)
    {
        window.location.replace("../level/game_win.html");
        clearTimeout(ch);
    }
}
